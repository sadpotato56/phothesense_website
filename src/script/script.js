...
document.addEventListener('DOMContentLoaded', () => {
  ...
  initMobileHeaderAutoHide();  // 2. Tự động ẩn header khi cuộn xuống (Mobile)
...
function initHeroHeaderScroll() {
  ...
  console.log("Check Header:", heroHeader);
  if (!heroHeader) return;
  const THRESHOLD = 60;
  const handleScroll = () => {
    // KIỂM TRA 2: In ra vị trí cuộn để xem sự kiện có chạy không
    // console.log("Đang cuộn:", window.scrollY); 
